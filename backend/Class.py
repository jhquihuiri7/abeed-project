...
class session_features:
    ...
    def read_data(self, feature_name_input, id_input, db_name_input, requirements_input, equation_input, alias_map_input, units_input, cumulative_input):
...
class Ops:
    ...
    def __init__(self) -> None:
        ...
        display_features_dict = {
            ...
            for feature in db_feature_list if feature.display_name
        }
    ...
    def add_db_data_features_button(self, db_data_features_input: list[str], overwrite_df=False, init_columns=[]):# TODO

        for feature in db_data_features_input:
            feature_obj = session_features()
            if feature in self.display_features_dict:
                feature_obj.read_data(feature, self.display_features_dict[feature].id, self.display_features_dict[feature].db_name, [], "", {}, self.display_features_dict[feature].units, False)
            elif feature in self.db_name_dict:
                feature_obj.read_data(feature, self.db_name_dict[feature].id, self.db_name_dict[feature].db_name, [], "", {}, self.db_name_dict[feature].units, False)
    ...
    def remove_data_features_button(self, remove_data_features_input: list[str]):# TODO
    ...
    def update_date_range_button(self, start_date_input: date, end_date_input: date):# TODO
    ...
    def create_custom_feature_button(self, equation_input: str, alias_map_input: dict, cumulative_input: bool = False, custom_name_input:str = None):#TODO needs to work with new custom feature format
        # Validation TODO:
        ...
        self.create_custom_feature(equation_input, alias_map_input, custom_name_input, cumulative_input)
        
        self.df[self.session_data_features[custom_name_input].feature_name] = self.create_custom_feature_column(self.df.copy(), self.session_data_features[custom_name_input])
        
        self.filter_df[self.session_data_features[custom_name_input].feature_name] = self.create_custom_feature_column(self.filter_df.copy(), self.session_data_features[custom_name_input])
    ...
    def update_db_data_features(self, overwrite_df=False, init_columns=[]):
        db_session_features = [value.db_name for key, value in self.session_data_features.items() if value.equation == ""]

        current_df = self.df.copy()
        self.df = simple_feature_request(self.start_date, self.end_date, db_session_features)[0]
        self.df.rename(columns=self.db_name_to_display_names_dict, inplace=True)
        if overwrite_df:
            init_columns = list(set(init_columns) - set(self.session_data_features.keys()))
            self.df = pd.concat([current_df[init_columns], self.df], axis=1)

    def update_created_data_features(self):
        dependent_features = [feature for feature in self.session_data_features if self.session_data_features[feature].equation != ""]
        while len(dependent_features) > 0:
            for feature in dependent_features:
                if feature not in list(self.df.columns) and all([requirement in list(self.df.columns) for requirement in self.session_data_features[feature].requirements]):
                    dependent_features.remove(feature)
                    self.df[feature] = self.create_custom_feature_column(self.df.copy(), self.session_data_features[feature])
        ...
        if not self.df.empty:
            ...
            for filter in self.feature_filters:
                ...
                dates_to_exclude_series = self.df.index[
                    (self.df[feature_name] <= min_value) | (self.df[feature_name] >= max_value)
                ]
    ...
    def update_filter_df(self):
        ...
        for feature in self.session_data_features:
            if self.session_data_features[feature].cumulative:
                self.filter_df = self.filter_df.drop(self.session_data_features[feature].feature_name, axis=1)
        ...
        for feature in self.session_data_features:
            if self.session_data_features[feature].feature_name not in self.filter_df.columns.to_list():   
                self.filter_df[self.session_data_features[feature].feature_name] = self.create_custom_feature_column(self.filter_df.copy(), self.session_data_features[feature])

    def create_custom_feature(self, equation, alias_map, feature_name, cumulative: bool):
        ...
        custom_feature_object.read_data(feature_name, None, None, requirements, equation, alias_map, units, cumulative)
    ...
    def create_custom_feature_column(self, df, session_feature_obj: session_features):
        ...
        local_dict = {var: df[feature].values for var, feature in variable_map.items()} #TODO does not work with filter_df_implementation when trying to recalculate cumulative features because it always calculates based on the self.df, not the self.filter_df
