...
def main_checkbox(client, id):
    ...
    options = (
        client.available_readable_names
        if id == "main_checkbox"
        else client.available_db_names[:45]
    )
...
def pagination(
    page_store_id, features_id, prev_id, next_id, pagination_id, is_main=False
):
    ...
    return html.Div(
        ...
        children=[
            ...
            html.Div(
                id=features_id, className="flex flex-row flex-wrap justify-between"
            ),
        ],
    )
